...
=======
[^2]: Achtung: Das ist natürlich abhängig davon, wie wir den Effekt definieren!
    Siehe Frage 2. Hier gehen wir davon aus, dass jemand weniger Zeit benötigt.
    Der Term `b * x` sollte entsprechend negativ werden, wenn er zu `a`
    hinzuaddiert wird wenn gestarrt wird und keinen Einfluss auf `a` haben, wenn
    nicht gestarrt wird.
>>>>>>> 897d8d48d0abbc7912d62d787ad2c2c25007c2ab
...
wir die **Wahrheit** über unser Model kennen (H~1~ ist wahr) wissen wir, das
...
* Wie viele Eyperimente wollen wir simulieren?
...
0. Stichprobengröße
0. Wahrscheinlichkeit Fehler 1. Art (*Signifikanzniveau*)
0. Wahrscheinlichkeit Fehler 2. Art (1-*Power*)
0. Effektgröße
...
nsims <- 100 # hat nur was mit der anzunehmenden Genauigkeit der Power zutun
...
* H~1~: Menschen benötigen `r effect_size` Sekunden weniger Zeit die Straße zu
  überqueren, wenn sie angestarrt werden), wir legen fest, dass wir in `r
  type_I_error * 100`% der Fälle H~1~ **fälschlicherweise** akzeptieren und wir
  legen fest, dass wir `r nsubjects` Versuchspersonen erheben.
